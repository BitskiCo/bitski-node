...
test('it successfully creates a provider', () => {
  ...
  const provider = manager.getProvider({network: 'rinkeby', disableBlockTracking: true});
...
test('it can pass additional headers', () => {
  expect.assertions(2);
  const manager = new ProviderManager('foo', 'bar');
  const provider = manager.getProvider({network: 'rinkeby', disableBlockTracking: true, additionalHeaders: { 'x-foo': 'bar' }});
  expect(provider).toBeDefined();
  // @ts-ignore
